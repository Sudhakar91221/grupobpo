...
export default class Autosearch extends PureComponent {
      ...
      .start(() => {
        ...
        if (this.mounted) {
          this.setState({ value, modal: false },() => {
            this.props.onClosePicker(value)
  ...
  renderItem({ item, index }) {
    ...
    return (
        ...
        >
          <Text style={[styles.item, itemTextStyle, textStyle,{paddingHorizontal:20}]} numberOfLines={1}>
            {title}
          </Text>
        </DropdownItem>
    );
      ...
      <View onLayout={this.onLayout} ref={this.updateContainerRef} style={containerStyle}>
       ...
       {modal == true &&
        <View
          ...
          style={containerStyle,{backgroundColor:'white',paddingTop:-20,borderBottomWidth:1,borderBottomColor:'gray',borderRightWidth:1,borderRightColor:'gray',borderLeftWidth:1,borderLeftColor:'gray'}}
          visible={modal}
          transparent={false}
        //   onRequestClose={this.onClosePicker}
        //   onResponderRelease={this.onClosePicker}
