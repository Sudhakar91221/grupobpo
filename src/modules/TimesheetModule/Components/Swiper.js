...
const styles = StyleSheet.create({
  ...
  prevButtonStyle: {
    ...
    paddingTop: 100,
    position: 'absolute',
    //paddingTop:100,height:200,width:'100%',backgroundColor:'green'
  },
  nextButtonStyle: {
    position: 'absolute',
    paddingTop: 100,
...
export default class Swiper extends React.Component {
  constructor(props) {
    ...
    this._panResponder = PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (e, gestureState) => {
        ...
        const allow =
          Math.abs(
            this.props.direction === 'row' ? gestureState.dx : gestureState.dy,
          ) > 5;
        if (allow) {
          this.stopAutoplay();
        }
  ...
  _fixState() {
    this._animatedValueX =
      this.props.direction === 'row'
        ? this.state.width * this.state.activeIndex * -1
        : 0;
    this._animatedValueY =
      this.props.direction === 'row'
        ? 0
        : this.state.height * this.state.activeIndex * -1;
        ...
        {!buttonsEnabled ? null : (
          <View
            style={[
              ...
              direction === 'row' ? {left: 0} : {paddingTop: 30},
              controlsWrapperStyle,
            ]}>
            <View style={{opacity: !activeIndex ? overRangeButtonsOpacity : 1}}>
              <TouchableOpacity
                disabled={!activeIndex && !loop}
                onPress={() => this.moveUpDown(true)}>
                {prevButtonElement || (
            ...
            <View
              style={{
                opacity:
                  activeIndex + 1 >= this.count ? overRangeButtonsOpacity : 1,
              }}>
              <TouchableOpacity
                disabled={activeIndex + 1 >= this.count && !loop}
                onPress={() => this.moveUpDown()}>
                {nextButtonElement || (
        ...
        <View style={[styles.sliderContainer, swipeAreaStyle]}>
          ...
          {!buttonsEnabled ? null : (
            <View
              ...
              <View
                style={{opacity: !activeIndex ? overRangeButtonsOpacity : 1}}>
                <TouchableOpacity
                  disabled={!activeIndex && !loop}
                  onPress={() => this.moveUpDown(true)}>
                  {prevButtonElement || (
                ...
                {children.map((el, i) => (
                  <View key={i}>
                    {i === activeIndex
                      ? activeDotElement || (
                          <View
                            style={[styles.activeDotStyle, activeDotStyle]}
                          />
                        )
                      : dotElement || (
                          <View style={[styles.dotStyle, dotStyle]} />
