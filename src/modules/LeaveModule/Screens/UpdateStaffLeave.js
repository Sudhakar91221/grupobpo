...
import {View, Alert, Text, ScrollView, FlatList} from 'react-native';
import {withTheme} from '../../../components/common/Theme/themeProvider';
import {GET_MY_BALANCE, UPDATE_STAFF_LEAVE} from '../Actions/type';
import {
  isLoadingSelector,
  apiSelector,
  errorSelector,
...
import {getMyBalance, updateStaffLeave} from '../Actions/LeaveActions';
...
class UpdateStaffLeave extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount() {
    ...
    this.state.userName = leaveDetail.userName;
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      (this.props.error.request == GET_MY_BALANCE ||
        this.props.error.request == UPDATE_STAFF_LEAVE)
    ) {
    ...
    //update leave
    if (this.props.api === UPDATE_STAFF_LEAVE) {
      if (this.props.error !== null && this.props.api === UPDATE_STAFF_LEAVE) {
        if (this.props.error !== prevProps.error) {
          Alert.alert(
            this.props.error.message,
            '',
      ...
      if (!this.props.error && this.props.api === UPDATE_STAFF_LEAVE) {
        if (this.props.updatedStaffLeaveId !== this.state.updatedStaffLeaveId) {
  ...
  onSubmitTapped() {
    ...
    var input = {
      ...
      userName: this.props.navigation.state.params.leaveDetail.userName,
      request: UPDATE_STAFF_LEAVE,
    };
    ...
    this.props.updateStaffLeave(input);
  }
...
const UpdateStaffLeaveNew = withTheme(UpdateStaffLeave);
UpdateStaffLeaveNew.navigationOptions = ({navigation, screenProps, params}) => {
...
function mapStateToProps(state) {
  return {
    ...
    updatedStaffLeaveId: updateStaffLeaveSelector(state.LeaveReducer),
  };
}
function mapDispatchToProps(dispatch) {
  return {
    getMyBalance: input => dispatch(getMyBalance(input)),
    updateStaffLeave: input => dispatch(updateStaffLeave(input)),
  };
}
