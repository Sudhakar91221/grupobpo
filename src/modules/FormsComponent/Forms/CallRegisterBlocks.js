...
class CallRegisterBlocks extends React.PureComponent {
  //MARK: - Constructor
  ...
  componentDidUpdate(prevProps) {
    if (!this.props.error && this.props.api === BLOCKS_GET) {
      if (this.props.blocks !== this.state.blocks) {
        ...
        if (
          this.props.blocks[0] !== undefined &&
          this.props.formId === this.props.blocks[0].formId
        ) {
    ...
    if (
      ...
      this.props.blockModel !== this.state.blockModel && this.props.blocks !== undefined
    ) {
  ...
  render() {
    ...
    if (this.props.blocks !== undefined) {
      ...
      if (
        ...
        this.state.blockModel.fbId !== lastRecivedBlockId
      ) {
        return <ActivityIndicatorCustom />;
      }
    }
    ...
    let submitApiName = 'Register';
    if (this.props.user !== undefined) {
      submitApiName = 'EditProfile';
    }
...
const CallRegisterBlocksNew = withTheme(CallRegisterBlocks);

CallRegisterBlocksNew.navigationOptions = ({
  navigation,
  screenProps,
  params,
}) => {
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTheme(CallRegisterBlocksNew));
