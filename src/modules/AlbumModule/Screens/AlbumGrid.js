  ...
  renderAlbumItem = ({item, index}) => {
    return (
      <AlbumCell
        ...
        isGrid={true}
        isLastItem={
          this.state.albumList.length % 2 === 0
            ? false
            : index === this.state.albumList.length - 1
            ? true
            : false
        }
      />
...
const AlbumGridNew = withTheme(AlbumGrid);
AlbumGridNew.navigationOptions = ({navigation, screenProps, params}) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(AlbumGridNew);
